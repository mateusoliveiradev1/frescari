...
function waitForTrpcMutation(page: Page, procedurePath: string): Promise<Response> {
...
test("produtor cadastra a fazenda e gerencia a frota operacional", async ({
    ...
    await expect(editVehicleDialog.locator("#fleet-vehicle-capacity")).toHaveValue(
        "900",
    );
...
test("produtor confirma a saida e registra override manual na mesa logistica", async ({
    ...
    await page.getByRole("button", { name: /^Priorizar$/ }).first().click();
...
test("produtor nao consegue excluir um veiculo que ja foi usado em uma saida confirmada", async ({
    ...
    await vehicleDialog
        .getByRole("button", { name: "Remover veiculo" })
        .click();
