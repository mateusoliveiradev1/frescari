...
function requestIncludes(request: { postData(): string | null; url(): string }, needle: string) {
...
async function mockBuyerTenantDetailPagination(page: Page) {
    ...
    await page.route("**/api/trpc/*", async (route) => {
        ...
        const procedurePath = new URL(request.url()).pathname.split("/api/trpc/")[1] ?? "";
...
async function mockTenantOverviewPagination(page: Page) {
    ...
    const firstPage = {
        ...
        tenants: Array.from({ length: 6 }, (_, index) => createTenantRecord(index + 1)),
    };
    const secondPage = {
        ...firstPage,
        nextCursor: null,
    ...
    await page.route("**/api/trpc/admin.getTenantOperationsOverview*", async (route) => {
        const request = route.request();
...
test("admin usuarios aplica o fix do scroll e envia os filtros corretos", async ({ page }) => {
    ...
    expect(
        consoleMessages.some(
            (message) =>
                ...
                message.includes("Detected `scroll-behavior: smooth` on the `<html>` element"),
        ),
    ).toBe(false);
});
...
test("admin usuarios abre o drill-down do tenant", async ({ page }) => {
    ...
    await expect(page.getByRole("link", { name: "Voltar para usuarios" })).toBeVisible();
...
test("admin usuarios carrega mais tenants no overview", async ({ page }) => {
    ...
    await expect(page.getByRole("button", { name: "Carregar mais tenants" })).toBeVisible();
    ...
    await expect(page.getByRole("button", { name: "Carregar mais tenants" })).toHaveCount(0);
});
