...
function addressFromSnapshot(
  snapshot: MetadataAddressSnapshot,
): MetadataAddress {
  return {
...
const alreadyProcessedExistingOrderStatuses = new Set<OrderStatus>([
  "payment_authorized",
  "awaiting_weight",
  "confirmed",
  "picking",
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const message =
      error instanceof Error ? error.message : "Stripe client unavailable";
    ...
    if (orderId) {
      try {
        await withWebhookBypass(async (database) => {
          await acquireTransactionLock(database, `order:${orderId}`);
          ...
          if (!targetOrder) {
            throw new Error(
              `Pedido ${orderId} nao encontrado para atualizacao do webhook.`,
            );
          }

          if (alreadyProcessedExistingOrderStatuses.has(targetOrder.status)) {
            console.log(
              `[WEBHOOK] Pedido ${orderId} ja processado no status ${targetOrder.status}. Ignorando duplicata.`,
            );
            return;
          }
          ...
          await database
            ...
            .where(eq(orders.id, orderId));
          console.log(
            `[WEBHOOK] Pedido ${orderId} atualizado para ${nextStatus}`,
          );
        });
      } catch (error) {
        console.error(
          `[WEBHOOK ERROR] Falha ao atualizar pedido ${orderId}:`,
          error,
        );
        ...
        return NextResponse.json(
          {
            received: true,
            error: "Database update failed for existing order",
          },
          { status: 200 },
        );
      }
      ...
      try {
        ...
        console.error(
          "[WEBHOOK ERROR] Falha ao processar checkout.session.completed:",
          error,
        );
...
async function handleCheckoutCompleted(session: Stripe.Checkout.Session) {
  ...
  console.log(
    `[WEBHOOK] Metadados extraídos: buyer=${buyerTenantId}, deliveryFee=${deliveryFeeStr}`,
  );

  // ── 2. Parse JSON safely ─────────────────────────────────────────
  let parsedItems: MetadataItem[];
  let parsedAddress: MetadataAddress;
  const parsedAddressSnapshot = parseAddressSnapshot(
    addressSnapshotRaw,
    session.id,
  );
  ...
  try {
    ...
    throw new Error(
      `[WEBHOOK] Failed to parse items JSON for session ${session.id}: ${itemsRaw}`,
    );
  }
  ...
  if (addressRaw) {
    try {
      ...
      throw new Error(
        `[WEBHOOK] Failed to parse address JSON for session ${session.id}: ${addressRaw}`,
      );
    }
  ...
  if (parsedItems.length === 0) {
    throw new Error(
      `[WEBHOOK] No items in metadata for session ${session.id}.`,
    );
  }

  const deliveryFee = parseFloat(deliveryFeeStr);
  console.log(
    `[WEBHOOK] Itens parseados: ${parsedItems.length} item(ns), deliveryFee=${deliveryFee}`,
  );
  ...
  if (!useLegacyCheckoutWebhookFlow) {
    const lockedFlowLotIds = [
      ...new Set(parsedItems.map((item) => item.lotId)),
    ].sort();
    const lockedFlowAddressLine =
      parsedAddressSnapshot?.formattedAddress ??
      buildDeliveryAddressLine(parsedAddress);
    const paymentIntentId =
      typeof session.payment_intent === "string"
        ? session.payment_intent
        : session.payment_intent?.id;
    ...
    console.log(
      `[WEBHOOK] Buscando lotes no banco: ${lockedFlowLotIds.join(", ")}`,
    );
    ...
    try {
      await withWebhookBypass(async (database) => {
        ...
        if (existingOrder.length > 0) {
          console.log(
            `[WEBHOOK] Sessao ${session.id} ja processada (order ${existingOrder[0]!.id}). Ignorando duplicata.`,
          );
          return;
        }
        ...
        if (fetchedLots.length !== lockedFlowLotIds.length) {
          throw new Error(
            `[WEBHOOK] Um ou mais lotes nao foram encontrados para IDs: ${lockedFlowLotIds.join(", ")}`,
          );
        }
        ...
        const itemsWithLotData = parsedItems.map((item) => {
          ...
          if (!lotData) {
            throw new Error(
              `[WEBHOOK] Lote ${item.lotId} nao encontrado no banco.`,
            );
          }
          ...
          return {
            ...
            saleUnit: resolveEffectiveSaleUnit(
              lotData.saleUnit,
              lotData.lotUnit,
            ),
        ...
        for (const sellerId of Object.keys(ordersBySeller)) {
          ...
          const hasWeightItems = sellerItems.some((item) =>
            isWeighableSaleUnit(item.saleUnit),
          );
          const initialStatus = hasWeightItems
            ? "awaiting_weight"
            : "confirmed";
          ...
          try {
            ...
            if (isPgUniqueViolation(error)) {
              throw new DuplicateCheckoutSessionError(
                `Sessao ${session.id} ja persistida para o vendedor ${sellerId}.`,
              );
            }
          ...
          if (!newOrder) {
            throw new Error(
              `[WEBHOOK] Falha ao inserir order para seller ${sellerId} - returning() vazio.`,
            );
          }
    ...
    console.log(
      `[WEBHOOK] Transacao completa para sessao ${session.id} | Comprador: ${buyerTenantId}`,
    );
    return;
  }

  // ── 3. Idempotency guard ─────────────────────────────────────────
  const existingOrder = await withWebhookBypass(async (database) =>
    database
      .select({ id: orders.id })
      .from(orders)
      .where(eq(orders.stripeSessionId, session.id))
  ...
  if (existingOrder.length > 0) {
    console.log(
      `[WEBHOOK] ⚠️ Sessão ${session.id} já processada (order ${existingOrder[0]!.id}). Ignorando duplicata.`,
    );
    return;
  }
  ...
  const fetchedLots = await withWebhookBypass(async (database) =>
    database
      .select({
        lotId: productLots.id,
        productId: productLots.productId,
  ...
  if (fetchedLots.length === 0) {
    throw new Error(
      `[WEBHOOK] Nenhum lote encontrado no banco para IDs: ${lotIds.join(", ")}`,
    );
  }
  ...
  const itemsWithLotData = parsedItems.map((item) => {
    ...
    if (!lotData) {
      console.warn(
        `[WEBHOOK] ⚠️ Lote ${item.lotId} não encontrado no banco. Será ignorado.`,
      );
    }
    return {
      ...item,
  ...
  try {
    await withWebhookBypass(async (database) => {
      ...
      const existingOrderInTransaction = await database
        .select({ id: orders.id })
        .from(orders)
        .where(eq(orders.stripeSessionId, session.id))
        .limit(1);

      if (existingOrderInTransaction.length > 0) {
        console.log(
          `[WEBHOOK] Sessao ${session.id} ja processada (order ${existingOrderInTransaction[0]!.id}). Ignorando duplicata.`,
        );
        return;
      }
      ...
      for (const sellerId of Object.keys(ordersBySeller)) {
        ...
        console.log(
          `[WEBHOOK] ✅ Pedido criado: ${newOrder.id} | Vendedor: ${sellerId} | Total: R$${orderTotal.toFixed(2)}`,
        );
        ...
        console.log(
          `[WEBHOOK] ✅ ${sellerItems.length} item(ns) inserido(s) para pedido ${newOrder.id}`,
        );
        ...
        for (const item of sellerItems) {
          ...
          console.log(
            `[WEBHOOK] ✅ Estoque decrementado: lote ${item.lotId} (-${item.qty})`,
          );
        }
    ...
    if (
      error instanceof DuplicateCheckoutSessionError ||
      isPgUniqueViolation(error)
    ) {
      console.warn(
        `[WEBHOOK] Sessao ${session.id} ja processada por outra transacao. Ignorando retry duplicado.`,
      );
      return;
    }
