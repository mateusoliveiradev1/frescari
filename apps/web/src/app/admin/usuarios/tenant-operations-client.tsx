...
type TenantOperationsOverview = RouterOutputs["admin"]["getTenantOperationsOverview"];
...
export function TenantOperationsClient() {
    const [health, setHealth] = useState<"ALL" | "inactive" | "needs_setup" | "operating">(
        "ALL",
    );
    const [type, setType] = useState<"ALL" | "BUYER" | "PRODUCER">("ALL");
    const [activityWindowDays, setActivityWindowDays] = useState<30 | 90>(30);

    const tenantOperationsQuery = trpc.admin.getTenantOperationsOverview.useInfiniteQuery(
        {
            activityWindowDays,
    ...
    const loadedTenants = tenantOperationsQuery.data?.pages.flatMap((page) => page.tenants) ?? [];
                    ...
                    <div className="max-w-3xl space-y-4">
                        ...
                        <div className="space-y-3">
                            ...
                            <p className="max-w-2xl font-sans text-sm leading-7 text-bark/76 sm:text-base">
                                Aqui a lente nao e faturamento. E prontidao operacional:
                                quem entrou, quem travou e quem ja esta rodando de verdade.
                            </p>
                        </div>
                    </div>
