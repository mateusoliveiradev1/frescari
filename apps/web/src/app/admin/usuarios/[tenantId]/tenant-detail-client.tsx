...
type TenantOrdersAsBuyerPage = RouterOutputs["admin"]["getTenantOrdersAsBuyerPage"];
type TenantOrdersAsSellerPage = RouterOutputs["admin"]["getTenantOrdersAsSellerPage"];
...
function EmptyState(props: { body: string; title: string }) {
    return (
        <div className="rounded-[24px] border border-dashed border-forest/14 bg-cream/40 px-5 py-8">
            ...
            <p className="mt-3 font-sans text-sm leading-6 text-bark/68">{props.body}</p>
        </div>
    );
}
        ...
        >
            <div className="flex items-start justify-between gap-4">
                <div className="space-y-2">
                    ...
                    <p className="font-sans text-sm leading-6 text-bark/72">{user.email}</p>
...
function AddressesList({ addresses }: { addresses: TenantAddressesPage["items"] }) {
...
export function TenantDetailClient({ tenantId }: { tenantId: string }) {
    ...
    const buyerOrdersQuery = trpc.admin.getTenantOrdersAsBuyerPage.useInfiniteQuery(
        {
            limit: DETAIL_LIST_PAGE_SIZE,
            tenantId,
        },
    ...
    const sellerOrdersQuery = trpc.admin.getTenantOrdersAsSellerPage.useInfiniteQuery(
        {
            limit: DETAIL_LIST_PAGE_SIZE,
            tenantId,
        },
    ...
    const addressItems = addressesQuery.data?.pages.flatMap((page) => page.items) ?? [];
    ...
    const buyerOrderItems = buyerOrdersQuery.data?.pages.flatMap((page) => page.items) ?? [];
    const sellerOrderItems = sellerOrdersQuery.data?.pages.flatMap((page) => page.items) ?? [];
                ...
                <div className="flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
                    <div className="max-w-3xl space-y-4">
                        ...
                        <div className="flex flex-wrap gap-3 text-xs uppercase tracking-[0.16em] text-bark/58">
                            ...
                            <span>
                                prontidao {detail.health.progressPercent}%
                            </span>
                        </div>
                    </div>
                            ...
                            <div className="flex flex-wrap gap-2">
                                {activityWindowOptions.map((option) => (
                                    <Button
                                        ...
                                        size="sm"
                                        variant={
                                            activityWindowDays === option.value
                                                ? "primary"
                                                : "ghost"
                                        }
                                    >
