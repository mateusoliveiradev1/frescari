...
export function AdminTrendChart({ metric, points }: AdminTrendChartProps) {
    ...
    const stepX =
        points.length > 1 ? (chartWidth - padding * 2) / (points.length - 1) : chartWidth / 2;
                ...
                <div className="mt-4 grid grid-cols-4 gap-2 text-center sm:grid-cols-7">
                    {points
                        .filter((_, index) => {
                            ...
                            return index % Math.ceil(points.length / 7) === 0 || index === points.length - 1;
                        })
                        .map((point) => (
                            <div key={point.day} className="rounded-[16px] bg-cream/45 px-2 py-2">
