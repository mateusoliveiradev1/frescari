...
export default async function SupplierRegionPage({
  ...
  const productRegionPathByKey = new Map(
    data.productRegions.map((region) => [
      `${region.categorySlug}:${region.productSlug}`,
      region.path,
    ]),
  );
  const localProducts = data.products.map((product) => {
    const localPath =
      productRegionPathByKey.get(`${product.categorySlug}:${product.slug}`) ??
      product.path;
  ...
  const categoryLinks =
    data.categoryRegions.length > 0
      ? data.categoryRegions.map((region) => ({
          path: region.path,
          label: region.categoryName,
        }))
  ...
  const commercialSnapshot = buildSupplierRegionCommercialSnapshot(data.lots, {
