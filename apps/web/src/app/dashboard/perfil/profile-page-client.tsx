...
import { LoaderCircle, MapPin, Pencil, Plus, Trash2, Truck, X } from "lucide-react";
import { toast } from "sonner";

import { Button, Skeleton, SkeletonText } from "@frescari/ui";
import { trpc } from "@/trpc/react";

type BuyerProfilePageClientProps = {
...
export default function BuyerProfilePageClient({
    userName,
}: BuyerProfilePageClientProps) {
    const utils = trpc.useUtils();
    const [editingAddressId, setEditingAddressId] = React.useState<string | null>(null);
    const [form, setForm] = React.useState<AddressFormState>(() => getInitialFormState());
    ...
    const defaultAddress = (defaultAddressQuery.data ?? null) as SavedAddress | null;
    ...
    return (
        <div className="min-h-screen bg-cream">
            <main className="mx-auto flex max-w-[1240px] flex-col gap-8 px-6 py-12 lg:px-12">
                <header className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
                    <div className="space-y-2">
                        ...
                        <h1 className="font-display text-4xl font-black text-soil">
                            Enderecos de entrega de{" "}
                            <span className="italic text-forest">{firstName}</span>
                        </h1>
                        <p className="max-w-2xl font-sans text-sm leading-6 text-bark/80">
                            O checkout usa o endereco padrao para calcular frete por fazenda.
                            O primeiro endereco salvo vira padrao automaticamente.
                        </p>
                    </div>
                                ...
                                {addressesQuery.isLoading ? (
                                    ...
                                    <div className="rounded-[20px] border border-dashed border-soil/15 bg-white/70 px-5 py-8 text-center">
                                        ...
                                        <p className="mx-auto mt-2 max-w-lg font-sans text-sm leading-6 text-bark/75">
                                            Assim que voce salvar o primeiro endereco, ele aparecera aqui
                                            com acoes de edicao e exclusao.
                                        </p>
                                    </div>
                                )}
                            </div>
                    ...
                    <section className="rounded-[24px_18px_22px_20px] border border-soil/8 bg-cream p-6 shadow-card">
                        ...
                        <form className="mt-6 space-y-4" onSubmit={handleSubmit}>
                            ...
                            <div className="flex flex-wrap gap-3 pt-2">
                                <Button className="flex-1 justify-center" disabled={isSubmitting} type="submit">
                                    {isSubmitting ? (
                                        <>
                                            <LoaderCircle className="h-4 w-4 animate-spin" />
                                            {isEditing ? "Atualizando endereco..." : "Salvando endereco..."}
                                        </>
                                    ) : isEditing ? (
                                        <>
                                            <Pencil className="h-4 w-4" />
