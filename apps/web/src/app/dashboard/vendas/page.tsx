...
function calculateWeighingPreview(
  order: ReceivedOrder,
  ...
  const itemSubtotalCents = order.items.reduce((sum, item) => {
    ...
    if (isWeightBasedItem) {
      maxByItemId[item.id] = roundToThreeDecimals(
        requestedQty * WEIGHT_SAFETY_MARGIN,
      );
    }
  ...
  return (
    <div className="space-y-3">
      {Array.from({ length: 5 }).map((_, i) => (
        <div
          key={i}
          className="flex items-center gap-4 p-5 bg-white border border-border rounded-lg animate-pulse"
        >
...
export default function VendasPage() {
  ...
  const [weighingOrder, setWeighingOrder] = useState<ReceivedOrder | null>(
    null,
  );
  const [weighingWeights, setWeighingWeights] = useState<
  ...
  const { data: fetchOrders, isLoading } =
    trpc.order.listReceivedOrders.useQuery({
      status: filterStatus,
    });
  ...
  const { mutate: captureWeight, isPending: isCapturing } =
    trpc.order.captureWeighedOrder.useMutation({
      onSuccess: (data) => {
        toast.success(
          `Pesagem confirmada. L\u00EDquido a receber: ${formatCurrency(data?.netAmount ?? data?.finalAmount ?? 0)}`,
        );
  ...
  const isPreviewStale =
    Boolean(weighingOrder) && deferredWeighingWeights !== weighingWeights;
  const hasInvalidWeighing = weighingOrder
    ? weighingOrder.items.some((item) => {
        ...
        const maxAllowedWeight = roundToThreeDecimals(
          Number(item.qty) * WEIGHT_SAFETY_MARGIN,
        );
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {[
            ...
            <button
              ...
              className={`px-4 py-1.5 rounded-full text-sm font-bold tracking-wide transition-colors border ${
                filterStatus === f.id
        ...
        {isLoading ? (
                        ...
                        <td className="py-4 px-6 text-center">
                          {order.status === "awaiting_weight" ? (
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
                                const initialWeights = order.items.reduce(
                                  (weights, item) => {
                                    if (isWeighableSaleUnit(item.saleUnit)) {
          ...
          <div className="py-24 border border-dashed border-soil/10 rounded-sm bg-white flex flex-col items-center justify-center gap-6 text-center shadow-card">
            <div className="w-20 h-20 rounded-full bg-sage/50 flex items-center justify-center mb-2">
              <PackageOpen
                className="w-10 h-10 text-forest/40"
                strokeWidth={1.5}
              />
            </div>
          ...
          <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl overflow-hidden">
            ...
            <div className="p-6 space-y-5 max-h-[70vh] overflow-y-auto">
                ...
                <div className="grid gap-3 sm:grid-cols-3">
                  <div className="rounded-lg border border-soil/10 bg-cream-dark/40 p-4">
                    ...
                    <p className="mt-1 text-[11px] text-bark/70">
                      Inclui entrega de{" "}
                      {formatCurrency(weighingPreview.deliveryFee)}
                    </p>
                  </div>
              ...
              {weighingOrder.items.map((item) => {
                ...
                return (
                  <div
                    key={item.id}
                    className="rounded-lg border border-soil/10 bg-white p-4 space-y-3"
                  >
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        ...
                        <p className="mt-1 text-[11px] text-bark/70">
                          Solicitado:{" "}
                          {Number(item.qty).toFixed(isWeight ? 3 : 0)}{" "}
                          {saleUnitLabel}
                        </p>
                      </div>
                      <span
                        className={`rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.12em] ${isWeight ? "bg-purple-100 text-purple-800" : "bg-sage/40 text-bark/70"}`}
                      >
                    ...
                    <div className="grid gap-3 sm:grid-cols-[1fr_auto] sm:items-center">
                      <div className="space-y-2">
                        <input
                          ...
                          value={
                            isWeight ? weighingWeights[item.id] || "" : item.qty
                          }
                          onChange={(e) =>
                            setWeighingWeights((prev) => ({
                              ...prev,
                              [item.id]: parseFloat(e.target.value) || 0,
                            }))
                          }
                          disabled={!isWeight}
                          className={`w-full rounded-sm border px-3 py-2 text-sm focus:outline-none focus:ring-1 ${
                            !isWeight
                    ...
                    {isOverLimit && (
                      <p className="text-[11px] font-medium text-red-600">
                        O peso informado ultrapassa o teto permitido de 10% para
                        este item.
                      </p>
                    )}
                  </div>
            ...
            <div className="flex flex-col gap-3 border-t border-soil/10 bg-cream-dark/30 p-4 sm:flex-row sm:items-center sm:justify-between">
              <p className="text-[11px] text-bark/70">
                A captura finaliza o pedido com os pesos informados e reconcilia
                o valor autorizado no Stripe.
              </p>
                    ...
                    const weighedItems = weighingOrder.items
                      ...
                      .filter(
                        (
                          ...
                          typeof item.finalWeight === "number" &&
                          item.finalWeight > 0,
                      );
