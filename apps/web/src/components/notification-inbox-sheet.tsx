...
function updateInboxNotifications(
  ...
  if (input.unreadOnly) {
    const nextNotifications = currentNotifications.filter(
      (notification) => !shouldMarkAsRead(notification),
    );
...
function applySingleReadToSummary(
  ...
  const nextByScope = {
    ...
    [notification.scope]: Math.max(
      0,
      currentSummary.byScope[notification.scope] - 1,
    ),
  };
...
function applyBulkReadToSummary(
  ...
  return {
    ...
    totalUnread: Math.max(
      0,
      currentSummary.totalUnread - currentSummary.byScope[scope],
    ),
...
export function NotificationInboxSheet({
  ...
  const unreadSummaryQuery = trpc.notification.getUnreadSummary.useQuery(
    undefined,
    {
      refetchInterval,
  ...
  const markReadMutation = trpc.notification.markRead.useMutation({
    onMutate: async ({ ids }) => {
      ...
      const targetNotification = notifications.find((notification) =>
      ...
      utils.notification.getUnreadSummary.setData(undefined, (currentSummary) =>
        applySingleReadToSummary(
          currentSummary as NotificationSummary | undefined,
          targetNotification,
        ),
      );
  ...
  const markAllReadMutation = trpc.notification.markAllRead.useMutation({
    onMutate: async (input) => {
      ...
      syncOptimisticInboxCaches(
        (notification) =>
          mutationScope ? notification.scope === mutationScope : true,
        readAt,
      );
    },
    onError: async () => {
        ...
        <SheetHeader className="px-6">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <SheetDescription>
                Inbox operacional do seu tenant, atualizado com polling
                autenticado.
              </SheetDescription>
            </div>
            <Badge
        ...
        <div className="mt-6 border-t border-soil/10 px-6 pt-4">
          ...
          <div className="mt-4 flex items-center justify-between gap-3">
            ...
            <Button
              ...
              disabled={
                selectedScopeCount === 0 || markAllReadMutation.isPending
              }
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <time className="shrink-0 text-[11px] font-semibold uppercase tracking-[0.12em] text-bark/70">
                        {formatSaoPauloDateBR(notification.createdAt, {
                          day: "2-digit",
                          month: "2-digit",
                          hour: "2-digit",
                          minute: "2-digit",
