...
test("buildAuthPasswordResetEmail includes the recipient name and reset url", () => {
...
test("sendAuthPasswordResetEmail logs the reset link outside production when delivery is not configured", async () => {
  const logEntries: string[] = [];
...
test("sendAuthPasswordResetEmail throws in production when delivery is not configured", async () => {
  writableEnv.NODE_ENV = "production";
  Reflect.deleteProperty(writableEnv, "RESEND_API_KEY");
  Reflect.deleteProperty(writableEnv, "AUTH_EMAIL_FROM");
...
test("sendAuthPasswordResetEmail sends the payload to Resend when configured", async () => {
  let capturedRequest: {
    input: RequestInfo | URL;
    init?: RequestInit;
  } | null = null;
  ...
  assert.deepEqual(payload.to, ["buyer@example.com"]);
  assert.equal(payload.reply_to, "suporte@frescari.com");
  assert.equal(payload.subject, "Redefina sua senha de acesso na Frescari");
  assert.match(payload.text, /abc123/);
});
