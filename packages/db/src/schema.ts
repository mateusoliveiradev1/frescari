...
type LegacyGeometryValue = string | { type: string; [key: string]: unknown };

// Keep the legacy geometry adapter for pre-existing flows outside farms.
const geometry = customType<{ data: LegacyGeometryValue; driverData: string }>({
  dataType() {
...
export const dispatchConfidenceEnum = pgEnum("dispatch_confidence", [
  "high",
  "medium",
  "low",
]);
export const dispatchOverrideActionEnum = pgEnum("dispatch_override_action", [
  "pin_to_top",
  "delay",
]);
...
export const dispatchWaveStatusEnum = pgEnum("dispatch_wave_status", [
  "confirmed",
  "departed",
  "cancelled",
]);
...
export const userLegalAcceptances = pgTable(
  "user_legal_acceptances",
  {
    ...
    source: text("source").default("sign_up_email").notNull(),
  },
  (table) => [
    index("user_legal_acceptances_user_idx").on(table.userId),
    uniqueIndex("user_legal_acceptances_user_version_uidx").on(
      table.userId,
      table.legalVersion,
    ),
  ],
...
export const farms = pgTable("farms", {
  ...
  baseDeliveryFee: numeric("base_delivery_fee", { precision: 10, scale: 2 })
    .default("0")
    .notNull(),
  pricePerKm: numeric("price_per_km", { precision: 10, scale: 2 })
    .default("0")
    .notNull(),
...
export const addresses = pgTable(
  ...
  (table) => [
    ...
    uniqueIndex("addresses_one_default_per_tenant")
      .on(table.tenantId)
...
export const orders = pgTable(
  "orders",
  {
    ...
    buyerTenantId: uuid("buyer_tenant_id")
      .references(() => tenants.id)
      .notNull(),
    sellerTenantId: uuid("seller_tenant_id")
      .references(() => tenants.id)
      .notNull(),
...
export const notifications = pgTable(
  ...
  (table) => [
    ...
    index("notifications_tenant_role_created_idx").on(
      table.tenantId,
      table.recipientRole,
      table.createdAt,
    ),
...
export const farmVehicles = pgTable(
  "farm_vehicles",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    tenantId: uuid("tenant_id")
    ...
    availabilityStatus: fleetVehicleStatusEnum("availability_status")
      .default("available")
      .notNull(),
    notes: text("notes"),
    createdAt: timestamp("created_at", { withTimezone: true })
...
export const deliveryDispatchOverrides = pgTable(
...
export const deliveryDispatchWaves = pgTable(
  "delivery_dispatch_waves",
  {
    ...
    recommendedVehicleType: fleetVehicleTypeEnum(
    ...
    ).$type<DispatchRecommendationSnapshot | null>(),
...
export const deliveryDispatchWaveOrders = pgTable(
  ...
  (table) => [
    ...
    uniqueIndex("delivery_dispatch_wave_orders_wave_order_unique").on(
      table.waveId,
      table.orderId,
    ),
  ],
...
export const farmVehiclesRelations = relations(
  farmVehicles,
  ({ one, many }) => ({
    tenant: one(tenants, {
...
export const deliveryDispatchOverridesRelations = relations(
...
export const deliveryDispatchWavesRelations = relations(
...
export const deliveryDispatchWaveOrdersRelations = relations(
