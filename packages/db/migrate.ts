...
import { applyFinalRlsState } from "./src/rls-finalizer";
...
if (!connectionString) {
  throw new Error(
    "DATABASE_ADMIN_URL or DATABASE_URL is required to run database migrations.",
  );
}
...
async function detectLegacyBaselineIndex() {
  ...
  const hasCoreSchema = ["tenants", "farms", "products", "orders"].every(
    (table) => publicTables.includes(table),
  );
  ...
  const hasPaymentIntentId = rows.some(
    (row) =>
      row.table_name === "orders" && row.column_name === "payment_intent_id",
  );
