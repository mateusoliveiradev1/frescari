...
export const tenantOperationsFilterSchema = z.object({
    ...
    health: z.enum(['ALL', 'inactive', 'needs_setup', 'operating']).default('ALL'),
...
function createCountMap<T extends string>(rows: Array<{ count: string; key: T | null }>) {
    return new Map(
        rows
...
export function paginateByCreatedAtCursor<T>(
    ...
    const visibleRows = decodedCursor
        ? sortedRows.filter(
              (row) =>
                  comparePaginationCursorValues(args.getCursorValue(row), decodedCursor) > 0,
          )
        : sortedRows;
...
function buildHealthRecord(args: {
    ...
    if (args.tenant.type === 'PRODUCER') {
        const checklist = [
            ...
            { done: args.activeLotCount > 0 || args.productCount > 0, label: 'catalogo' },
        ];
        ...
        return {
            ...
            progressPercent: Math.round((countDoneSteps(checklist.map((item) => item.done)) / 3) * 100),
    ...
    const health: TenantHealthCode =
        args.addressCount === 0 ? 'needs_setup' : args.buyerOrderCount === 0 ? 'inactive' : 'operating';
    const healthLabel =
        args.addressCount === 0
            ? 'Sem endereco'
    ...
    return {
        ...
        healthLabel,
        productCount: 0,
        progressPercent: Math.round((countDoneSteps(checklist.map((item) => item.done)) / 2) * 100),
        sellerOperationalOrderCount: 0,
        stripeConnected: false,
        tenant: args.tenant,
        userCount: args.userCount,
...
export function buildTenantOperationsOverviewFromRecords(args: {
    ...
    const visibleTenants = args.allTenants.filter((tenant) => matchesFilters(tenant, args.filters));
    ...
    return {
        ...
        queues: {
            buyersWithoutAddress: visibleTenants
                .filter((tenant) => tenant.tenant.type === 'BUYER' && tenant.addressCount === 0)
                .slice(0, 5),
            producersWithoutFarm: visibleTenants
                .filter((tenant) => tenant.tenant.type === 'PRODUCER' && tenant.farmCount === 0)
                .slice(0, 5),
        ...
        summary: {
            buyersActive: visibleTenants.filter(
                (tenant) => tenant.tenant.type === 'BUYER' && tenant.health === 'operating',
            ).length,
            ...
            producersNeedingSetup: visibleTenants.filter(
                (tenant) => tenant.tenant.type === 'PRODUCER' && tenant.health === 'needs_setup',
            ).length,
            producersOperating: visibleTenants.filter(
                (tenant) => tenant.tenant.type === 'PRODUCER' && tenant.health === 'operating',
            ).length,
...
export async function getTenantOperationsOverview(db: AppDb, filters: TenantOperationsFilters) {
    ...
    const baseTenants = await db.select().from(tenants).orderBy(desc(tenants.createdAt));
    ...
    const allTenants = baseTenants.map((tenant) =>
        buildHealthRecord({
            ...
            sellerOperationalOrderCount: sellerOperationalOrderCounts.get(tenant.id) ?? 0,
...
export async function getTenantOperationDetail(db: AppDb, input: TenantOperationDetailInput) {
    ...
    const [productCountRow] = await db
        .select({ count: sql<string>`count(*)` })
        .from(products)
        .where(and(eq(products.tenantId, input.tenantId), eq(products.isActive, true)));
    ...
    const sellerOperationalOrderCount = toNumber(sellerOperationalOrderCountRow?.count);
...
export async function getTenantUsersPage(db: AppDb, input: TenantOperationPageInput) {
...
export async function getTenantFarmsPage(db: AppDb, input: TenantOperationPageInput) {
...
export async function getTenantAddressesPage(db: AppDb, input: TenantOperationPageInput) {
...
export async function getTenantProductsPage(db: AppDb, input: TenantOperationPageInput) {
...
export async function getTenantLotsPage(db: AppDb, input: TenantOperationPageInput) {
...
export async function getTenantOrdersAsBuyerPage(db: AppDb, input: TenantOperationPageInput) {
...
export async function getTenantOrdersAsSellerPage(db: AppDb, input: TenantOperationPageInput) {
    return getTenantOrdersPage(db, {
        ...input,
        side: 'seller',
    });
