...
const allowedProducerStatusTransitions: Record<
  string,
...
function assertOrderStatusTransition(
  ...
  const allowedNextStatuses =
    allowedProducerStatusTransitions[currentStatus] ?? [];
...
export const orderRouter = createTRPCRouter({
        ...
        return await db.transaction(async (tx) => {
          ...
          for (const items of Object.values(ordersBySellerFarm)) {
            ...
            for (const i of items) {
              const currentLot = fetchedLots.find(
                (lot) => lot.lotId === i.lotId,
              );
              ...
              const newQty = (
                Number(currentLot.availableQty) - i.quantity
              ).toString();
        ...
        return await db.transaction(async (tx) => {
          ...
          if (targetOrder.status !== "confirmed") {
            throw new TRPCError({
              code: "BAD_REQUEST",
              message:
                'Apenas pedidos com status "Processando" podem ser cancelados.',
            });
          }
  ...
  captureWeighedOrder: producerProcedureNoTransaction
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const { targetOrder, items } = await withAuthenticatedTransaction(
        db,
        user,
        async (tx) => {
      ...
      if (
        !["awaiting_weight", "payment_authorized", "confirmed"].includes(
          targetOrder.status,
        )
      ) {
        throw new TRPCError({
          ...
          payload.weighedItems.forEach((weighedItem, index) => {
            if (!weightItemIds.has(weighedItem.orderItemId)) {
              validationContext.addIssue({
                ...
                message:
                  "Apenas itens vendidos por peso podem ser enviados para captura.",
              });
              return;
            }
      ...
      const finalWeightsByItemId = new Map(
        validatedPayload.data.weighedItems.map(
          ({ orderItemId, finalWeight }) => [orderItemId, finalWeight],
        ),
      );
      ...
      const deliveryFeeCents = Math.round(
        Number(targetOrder.deliveryFee) * 100,
      );
      ...
      const storedTotalCents = Math.round(
        Number(targetOrder.totalAmount) * 100,
      );
      ...
      const capturedWeightFingerprint =
        paymentIntent.metadata?.[STRIPE_CAPTURE_FINGERPRINT_METADATA_KEY] ??
        null;
      ...
      if (alreadyCaptured) {
        ...
        if ((paymentIntent.amount_capturable ?? 0) < grossAmountCents) {
          throw new TRPCError({
            code: "BAD_REQUEST",
            message:
              "O valor final ultrapassa o total previamente autorizado. Revise os pesos informados.",
          });
        }

        const idempotencyKey = `capture-weighed-order:${targetOrder.id}:${createHash(
          "sha256",
        )
        ...
        const captureParams: Stripe.PaymentIntentCaptureParams = {
          ...
          metadata: {
            ...
            [STRIPE_CAPTURE_FINGERPRINT_METADATA_KEY]: captureFingerprint,
        ...
        try {
          await getStripeClient().paymentIntents.capture(
            paymentIntentId,
            captureParams,
            {
              idempotencyKey,
