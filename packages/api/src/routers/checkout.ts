...
export const checkoutRouter = createTRPCRouter({
    ...
    createFarmCheckoutSession: buyerProcedure
        ...
        .mutation(async ({ ctx, input }) => {
            ...
            const wrongFarmLot = lotDataForStripe.find((lot) => lot.farmId !== input.farmId);
            if (wrongFarmLot) {
                throw new TRPCError({
                    code: 'BAD_REQUEST',
                    message: 'Todos os itens do checkout devem pertencer a fazenda selecionada.',
                });
            }
            ...
            if (!freightQuote.hasReachedMinimumOrder) {
                throw new TRPCError({
                    code: 'PRECONDITION_FAILED',
                    message:
                        `Este pedido nao atingiu o valor minimo de ${formatCurrencyMessage(freightQuote.minOrderValue)} para esta fazenda.`,
                });
            }
            ...
            const addressSnapshotJson = JSON.stringify(buildAddressSnapshot(addressRecord));
