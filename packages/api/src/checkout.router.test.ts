...
const stripeState = {
    createdSessionPayload: null as Record<string, any> | null,
...
test('checkout.createFarmCheckoutSession recalculates freight on the server and stores address snapshot metadata', async () => {
    ...
    assert.equal(payload.metadata.farm_id, '22222222-2222-4222-8222-222222222222');
    assert.equal(payload.metadata.address_id, '11111111-1111-4111-8111-111111111111');
    ...
    const addressSnapshot = JSON.parse(payload.metadata.address_snapshot) as Record<string, unknown>;
...
test('checkout.createFarmCheckoutSession rejects lots that do not belong to the informed farm', async () => {
    ...
    await assert.rejects(
        () =>
            (caller as Record<string, any>).checkout.createFarmCheckoutSession({
                farmId: '22222222-2222-4222-8222-222222222222',
                addressId: '11111111-1111-4111-8111-111111111111',
                items: [
                    {
...
test('checkout.createFarmCheckoutSession supports platform-only Stripe mode without a producer Connect account', async () => {
    ...
    try {
        ...
        assert.equal(payload.payment_intent_data?.application_fee_amount, undefined);
        assert.equal(payload.payment_intent_data?.metadata?.stripe_connect_mode, 'platform_only');
...
test('stripe.createStripeConnect fails fast when platform-only Stripe mode is enabled', async () => {
    ...
    try {
        ...
        await assert.rejects(
            () => (caller as Record<string, any>).stripe.createStripeConnect({}),
