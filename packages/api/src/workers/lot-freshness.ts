...
export function calculateFreshnessScore(
    ...
    const lifetimeMs = normalizedExpiryDate.getTime() - normalizedHarvestDate.getTime();
...
export async function runLotFreshnessPass({
    ...
    for (const lot of activeLots) {
        const freshnessScore = calculateFreshnessScore(lot.harvestDate, lot.expiryDate, now);
...
export function createBullLotEventPublisher(queue: LotEventQueue): LotEventPublisher {
    return {
