...
const nullableOptionalTrimmedString = (fieldLabel: string, minLength = 1) =>
  z.preprocess(
    ...
    z
      .string()
      .trim()
      .min(minLength, `${fieldLabel} invalido.`)
      .nullable()
      .optional(),
  );
...
const normalizedZipcodeSchema = z.preprocess(
  (value) =>
    typeof value === "string" ? normalizeBrazilianPostalCode(value) : value,
  z
    .string()
    .trim()
    .regex(brazilianPostalCodeRegex, "CEP invalido (ex: 01234-567)."),
);
...
export const insertFarmSchema = z.object({
  ...
  deliveryRadiusKm: nonNegativeIntegerSchema(
    "Raio maximo de entrega",
  ).optional(),
  ...
  freeShippingThreshold: nonNegativeNumberSchema("Frete gratis a partir de")
    .nullable()
    .optional(),
...
export const selectFarmSchema = z.object({
  ...
  freeShippingThreshold: nonNegativeNumberSchema(
